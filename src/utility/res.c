...
//res_open("gotres.001");
//memset(name,0,1024);
//res_read_element("speech",(char far*) name,0l,200l);
//printf(name);
//res_close();
//exit(0);
...
if(!strcmp(argv[2],"C")){
  ...
  for(i=0;i<RES_MAX_ENTRIES;i++){
     if(res_header[i].length){
       ...
       printf("%s  %.6ld  %.6ld   %.6ld   %.3d\r\n",name,res_header[i].offset,
              res_header[i].length,res_header[i].original_size,
              res_header[i].key);
     }
  }
  res_abort();
  ...
  for(i=0;i<RES_MAX_ENTRIES;i++){
     if(res_header[i].length){
       ...
       strcat(path,".DAT ");
       strcat(path,fname);
       if(res_header[i].key) strcat(path," 1\n");
       else strcat(path," 0\n");
       fputs(path,fp);
     }
  }
