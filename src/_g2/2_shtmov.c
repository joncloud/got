...
//===========================================================================
int check_movewb(int x,int y,ACTOR *actr){

if(x<16 || x>287 || y<1 || y>159) return 0;
actr->x=x;
actr->y=y;
return 1;
}
...
check_movewb(actr->x,actr->y,actr);
next_frame(actr);
actr->last_dir=d;
if(actr->directions==1) return 0;
return d;
...
//===========================================================================
int shot_movement_twelve(ACTOR *actr){  //angle throw loki
int x1,y1;
...
if(x1<16 || x1>287 || y1<16 || y1>159){
  calc_angle(rand()%319,rand()%191,x1,y1,actr);
  actr->move=13;
  actr->temp4=240;
  actr->next=2;
}
else{
  if(overlap(x1+2,y1+2,x1+14,y1+14,thor_x1,thor_y1,thor_x2,thor_y2)){
    thor_damaged(actr);
  }
  actr->x=x1;
  actr->y=y1;
...
//===========================================================================
int shot_movement_thirteen(ACTOR *actr){  //angle throw loki-2
int x1,y1;
...
if(x1<16 || x1>287 || y1<16 || y1>159){
  ...
  if(overlap(x1+4,y1+4,x1+12,y1+12,thor_x1,thor_y1,thor_x2,thor_y2)){
    thor_damaged(actr);
  }
  actr->x=x1;
  actr->y=y1;
