...
extern char slip_flag;
extern char slip_cnt;
extern char slipping;
extern char eyeballs;

char *object_names[]={"Shrubbery","Child's Doll","UNUSED",
...
f=1;
if(new_level==BOSS_LEVEL){
  if(!setup.boss_dead[1]){
    if(!auto_load) boss_level();
    f=0;
  }
}
